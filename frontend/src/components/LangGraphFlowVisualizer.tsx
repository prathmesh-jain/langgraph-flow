...
export interface LangGraphFlowVisualizerProps {
  /** Backend API base URL (defaults to VITE_API_BASE_URL or localhost:8000) */
  baseUrl?: string;
  /** Graph topology endpoint path (defaults to /demo/graph) */
  graphEndpoint?: string;
  /** Stream endpoint path (defaults to /demo/stream) - used for node status updates */
  streamEndpoint?: string;
  /** Thread ID for connecting to the stream - obtained after calling run endpoint externally */
  threadId?: string;
  /** Custom API client for making requests (defaults to fetch) */
  apiClient?: {
    get: (url: string) => Promise<Response>;
  };
  /** Whether to show subgraphs (defaults to true) */
  showSubgraphs?: boolean;
  /** Whether to hide subgraphs after processing completes (defaults to false) */
  hideSubgraphAfterProcess?: boolean;
  /** Whether to auto-expand subgraphs when they start running (defaults to true) */
  autoExpandSubgraphs?: boolean;
  /** Whether to show the graph visualization (defaults to true) */
  showGraph?: boolean;
...
const DEFAULT_GRAPH_ENDPOINT = '/demo/graph';
const DEFAULT_STREAM_ENDPOINT = '/demo/stream';

// Create default fetch-based API client
...
function computeLayout(
  ...
  layers.forEach((layer, layerIndex) => {
    ...
    const startX = (canvasWidth - layerWidth) / 2 + (nodeWidths[layer[0]] || baseNodeWidth) / 2;
  ...
  sortedParentIds.forEach(parentId => {
    ...
    parentSubgraphNodes.forEach((subNode, index) => {
      if (expandedSubgraphs.has(parentId)) {
        const offsetY = (index + 1) * nodeSpacing;
...
export default function LangGraphFlowVisualizer({
  baseUrl = DEFAULT_BASE_URL,
  graphEndpoint = DEFAULT_GRAPH_ENDPOINT,
  streamEndpoint = DEFAULT_STREAM_ENDPOINT,
  threadId,
  apiClient = defaultApiClient,
  showSubgraphs = true,
  hideSubgraphAfterProcess = false,
  autoExpandSubgraphs = true,
  showGraph = true,
  onError,
}: LangGraphFlowVisualizerProps) {
  ...
  // Internal state for node status
  ...
  // Load graph topology from backend on mount
  useEffect(() => {
    if (!showGraph || topologyLoaded) return;

    const loadTopology = async () => {
      try {
        const response = await apiClient.get(`${baseUrl}${graphEndpoint}`);
        const topologyData: GraphTopology = await response.json();

        setTopology(topologyData);

        const layoutPositions = computeLayout(topologyData.nodes, topologyData.edges, expandedSubgraphs);

        const visibleNodes = topologyData.nodes.filter(node => {
          if (!showSubgraphs && node.type === 'subgraph') {
            return false;
          }
        ...
        const visibleEdges = topologyData.edges.filter(edge => {
          if (!showSubgraphs && edge.subgraph) {
            return false;
          }
          if (edge.subgraph) {
    ...
    loadTopology();
  }, [showGraph, topologyLoaded, baseUrl, graphEndpoint, apiClient, showSubgraphs, expandedSubgraphs, nodeStatus, setNodes, setEdges, onError]);

  // Stream node status updates when threadId is provided
  useEffect(() => {
    if (!threadId || !streamEndpoint) return;

    const streamNodeStatus = async () => {
      try {
        const separator = streamEndpoint.includes('?') ? '&' : '?';
        const url = `${baseUrl}${streamEndpoint}${separator}thread_id=${encodeURIComponent(threadId)}`;
        ...
        console.error('Failed to stream node status:', e);
        onError?.(e as Error);
      }
    };

    streamNodeStatus();
  }, [threadId, streamEndpoint, baseUrl, onError]);
  ...
  useEffect(() => {
    ...
    Object.entries(nodeStatus).forEach(([nodeId, status]) => {
      ...
      if (hideSubgraphAfterProcess && isSubgraphParent && status === 'completed' && newExpanded.has(nodeId)) {
  ...
  }, [nodeStatus, autoExpandSubgraphs, expandedSubgraphs, dynamicNodes, dynamicEdges, topology, hideSubgraphAfterProcess]);
  ...
  useEffect(() => {
    ...
    const flowNodes: Node[] = visibleNodes.map(node => {
      ...
      const label = node.type === 'subgraph' ? node.id.split('.')[1] || node.id : node.id;
      const labelLength = label.length;
      const calculatedWidth = Math.max(nodeWidth, Math.min(labelLength * 8 + 40, 300));
    ...
    setNodes(flowNodes);
    setEdges(flowEdges);
  }, [topology, topologyLoaded, expandedSubgraphs, dynamicNodes, dynamicEdges, nodeStatus, showSubgraphs, setNodes, setEdges]);
  ...
  return (
    <div className="h-[600px] bg-white rounded-lg shadow-md border">
      {topologyLoaded ? (
        ...
        <div className="h-full flex items-center justify-center">
          <p className="text-gray-500">Loading graph topology...</p>
        </div>
      )}
    </div>
  );
